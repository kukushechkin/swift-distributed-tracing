...
public protocol Tracer: LegacyTracer {
    ...
    ///
    /// The current task-local `TracingContext` is picked up and provided to the underlying tracer.
    ...
    /// - Returns: The span identified by the given `TracingContext` in case it's still recording.
    func activeSpan(identifiedBy context: TracingContext) -> Span?
}
...
extension Tracer {
    ...
    public func startSpan(
        _ operationName: String,
        context: @autoclosure () -> TracingContext = .current ?? .topLevel,
    ...
    /// - Parameter context: The context containing information that uniquely identifies the span being obtained.
    /// - Returns: `nil`.
    public func activeSpan(identifiedBy context: TracingContext) -> Span? {
        nil
    }
}
...
extension Tracer {
    ...
    public func withSpan<T>(
        ...
        defer { span.end() }
        do {
            return try TracingContext.$current.withValue(span.context) {
                try operation(span)
            }
        } catch {
    ...
    public func withSpan<T>(
        ...
        defer { span.end() }
        do {
            return try await TracingContext.$current.withValue(span.context) {
